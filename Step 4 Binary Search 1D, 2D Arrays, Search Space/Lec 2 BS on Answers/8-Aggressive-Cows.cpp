/*
Aggressive Cows :  (min distance b/w cows ) is max
...
int Linear(vector<int> &stalls, int k)
{
    int n = stalls.size();
    sort(stalls.begin(), stalls.end());
    int limit = stalls[n - 1] - stalls[0];
    // ans 1 se lekar limit tak ho skta hai means we have only two cows so
    // first cows on mini and another on max  to ans is max-min;

    for (int i = 1; i <= limit; i++)
    {
        if (canWePlace(stalls, i, k) == false)
            return i - 1; // MEANS AB WALA SABHI COWS KO PLACE NHI KARA TO LAST WALA SAHI HAI 
    }
    return limit;  // MEANS SABHI KO TRUE RETURN AAYA MEANS 2 COWS HAI LIMIT ANS 

    // TC O(LIMIT*N) + 0(NlogN) = O(LIMIT*N)
    // NlogN is for sort fuunction
    // SC O(1)
}


int Binary(vector<int> stalls,int k)
{
    int n=stalls.size();
    sort(stalls.begin(), stalls.end());
    int low=1;
...
int main()
{
    ...
    int k = 4;
    int ans = Linear(stalls, k);
    cout << "The maxLinearimum possible minimum distance is: " << ans << "\n";
    cout << "The maxLinearimum possible minimum distance is: " << Binary(stalls,k) << "\n";
    return 0;
}
